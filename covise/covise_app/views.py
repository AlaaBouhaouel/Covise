...
def login_view(request):
    ...
    if existing_user is None: #There is no user with this email yet
        ...
        if approved_waitlist_entry is not None:
            print("This email ", email, " comes from approved waitlist, but does not have an account yet: ", approved_waitlist_entry)
            context["error_message"] = "Your email is approved. But you need to create your account from sign in first."
        else:
            print("This email ", email, " does not have an account, and it is not in the approved waitlist either.")
    ...
    login(request, user)
    print("User ", email, " authenticated and logged in successfully.")
